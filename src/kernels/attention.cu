...
static void flash_attention_decode_impl(
...
static void flash_attention_prefill_batched_impl(
    half* output, const half* q, const void* k_cache, const void* v_cache,
    int n_heads, int n_kv_heads, int head_dim,
    int N, int start_pos,
    float scale, bool kv_int8,
...
Status flash_attention_decode(const AttentionDecodeArgs& args) {
    flash_attention_decode_impl(
        static_cast<half*>(args.output),
        static_cast<const half*>(args.q),
        args.k_cache,
        args.v_cache,
        args.n_heads,
        args.n_kv_heads,
...
Status flash_attention_prefill_batched(const AttentionPrefillBatchedArgs& args) {
    flash_attention_prefill_batched_impl(
        static_cast<half*>(args.output),
        static_cast<const half*>(args.q),
        args.k_cache,
        args.v_cache,
