...
static void gemv_q4(half* y, const void* W_q4, const half* scales,
                    const half* x, int M, int K, int group_size,
                    cudaStream_t stream) {
    (void)scales;
    (void)group_size;
    gemv_quant_impl(y, W_q4, 12, x, M, K, stream, nullptr);
}
...
static cudaStream_t cuda_stream(StreamHandle stream) {
...
Status gemm_quant_batched(const GemmQuantBatchedArgs& args) {
    gemm_quant_batched_impl(static_cast<half*>(args.y), args.weights,
                            ...
                            args.M, args.N, args.K, cuda_stream(args.stream),
                            args.weights_device);
...
Status gemv_quant_add(const GemvQuantAddArgs& args) {
    gemv_quant_add_impl(static_cast<half*>(args.y), args.weights,
                        args.ggml_type, static_cast<const half*>(args.x),
...
Status dequant_embedding_row(const DequantEmbeddingRowArgs& args) {
    if (args.hidden_dim <= 0 || args.hidden_dim % QK_K != 0) {
        return Status::InvalidArgument;
    }

    const void* W_device = weight_device_or_resolve(args.weights,
                                                    args.weights_device);
    if (!W_device) {
        return Status::InvalidArgument;
    }

    half* dst = static_cast<half*>(args.dst);
    const int blocks = args.hidden_dim / QK_K;
    switch (args.ggml_type) {
        case 12:
            dequant_embedding_q4k_kernel<<<blocks, QK_K, 0,
                                            cuda_stream(args.stream)>>>(
                dst, (const block_q4_K*)W_device, args.token_id,
                args.hidden_dim);
            break;
        case 14:
            dequant_embedding_q6k_kernel<<<blocks, QK_K, 0,
                                            cuda_stream(args.stream)>>>(
                dst, (const block_q6_K*)W_device, args.token_id,
                args.hidden_dim);
            break;
        default:
            return Status::UnsupportedQuantFormat;
    }
